...
def import_hr ():
    ...
    """
    # Get variables first
    rh_data_file = getenv("RH_DATA_FILE")
    sports_data_file = getenv("SPORTS_DATA_FILE")
    postgres_host = getenv("POSTGRES_DB_HOST")
    postgres_dbname = getenv("POSTGRES_DB_NAME")
    postgres_user = getenv("POSTGRES_ADMIN_USER")
    postgres_password = getenv("POSTGRES_ADMIN_PWD")
    postgres_connstring = getenv("SPORT_DATA_SQL_ALCHEMY_CONN")
    # Read HR data
    df_hr = pd.read_excel(rh_data_file)
    # Rename columns
    ...
    # Read sports data
    df_employee_sports = pd.read_excel(sports_data_file)
    ...
    db = create_engine(postgres_connstring)
    conn = db.connect()
